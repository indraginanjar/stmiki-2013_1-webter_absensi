...
function EditData(id){
	jQuery.ajax({
		async:"true",
        	...
        	dataType: "json",
		success: function (data, status, jqXHR) {
			$('#Nomor').val(data.properties.nofaktur);
			$('#Supplier').val(data.properties.kodesupp);
			AfterInputSupplier();
			var Tanggal = data.properties.tanggal;
			var Arr = Tanggal.split('-');
			Tanggal = Arr[2] + '/' + Arr[1] + '/' + Arr.properties;
			$('#Tanggal').val(Iso8601DateToIndonesia(data.properties.tanggal));
			//DrawDataTable(data);
		},
